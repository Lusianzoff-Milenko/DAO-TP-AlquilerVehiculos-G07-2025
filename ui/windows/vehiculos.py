...
def _load_data():
    ...
    def task():
        return _controller.get_all_vehiculos()

    run_async(task, on_success=_update_table)
...
def register():
    ...
    with dpg.child_window(tag=_TAG, parent="content_area", show=False, width=-1, height=-1):
        ...
        _table_view = TableView(
            ...
            on_edit=lambda row: _form_dialog.show(row),  # Falta implementar mapeo inverso ID->Label en show
            on_delete=_on_eliminar
        )
        _table_view.render(_TAG)

        # Definición del formulario
        # Nota: Las options se deberían llenar dinámicamente, aquí pongo placeholders
